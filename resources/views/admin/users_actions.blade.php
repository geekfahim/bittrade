                ...
                <div class="modal-body bg-{{Auth('admin')->User()->dashboard_style}}">
                        ...
                        <input style="padding:5px;" class="form-control bg-{{Auth('admin')->User()->dashboard_style}} text-{{$text}}" value="{{$list->name}}" type="text" disabled><br/>
                            ...
                            <div class="form-group">
                                ...
                                <option value="Ref Bonus">Ref Bonus</option>
...
<!-- Edit user Modal -->
    ...
    <!-- /Delete user Modal -->
     ...
     <!-- Verify useraccount Modal -->
    <div id="verifyUserModal{{$list->id}}" class="modal fade" role="dialog">
        <div class="modal-dialog">
            <!-- Modal content-->
            <div class="modal-content">
                <div class="modal-header bg-{{Auth('admin')->User()->dashboard_style}}">
                    <h4 class="modal-title text-{{$text}}">Are you sure you want to verify {{$list->name}} {{$list->l_name}}.</strong></h4>
                    <button type="button" class="close text-{{$text}}" data-dismiss="modal">&times;</button>
                </div>
                <div class="modal-body bg-{{Auth('admin')->User()->dashboard_style}}">
                    <a class="btn btn-{{$text}}" href="{{ url('admin/dashboard/verifysystemuser') }}/{{$list->id}}">Verify</a>
                </div>
            </div>
        </div>
    </div>
