				...
				<div class="page-inner">
					...
					<div class="row">
						<div class="col card bg-{{$bg}} shadow-lg p-4">
														  ...
														  <script type="text/javascript">
															...
															stripe.confirmCardPayment(clientSecret, {
																...
																if (result.paymentIntent.status === 'succeeded') {
																	...
																	$.ajax({
																		url: "{{url('/dashboard/submit-wallet-stripe-payment')}}",
																		type: 'GET',
																		data:$('#selectform').serialize(),
																		success: function (data) {
																			//location.reload(true);
																			$('#stripesuccess').removeClass("hidden");
																			window.location.replace("{{route('shopping_wallet')}}");
																		},
																		error: function (data) {
							...
							<div>
								<form method="post" action="{{ route('save_wallet_deposit') }}" enctype="multipart/form-data">
										<h5 class="text-{{$text}}">Upload Payment proof after payment. (Ignore if paid with card).</h5>
										<input type="file" name="proof" class="form-control col-lg-4 bg-{{$bg}} text-{{$text}}" accept="image/*">
									<br>
