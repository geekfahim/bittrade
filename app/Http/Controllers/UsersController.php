...
public function getdownlines($array, $parent = 0, $level = 1) {
  ...
  foreach ($array as $entry) {
      if ($entry->ref_by == $parent) {
        ...
        if($level==1){
          $levelQuote= "Direct referral level 1";
        }else{
...
function verifyaccount() {
  ...
  //add earnings to agent balance
          ...
          //create history
            ...
            'plan' => "Credit",
             ...
             'fromUser' => auth::user()->id,
              'remark' => "Level 1 Ref Bonus From self"
            ]);
            ...
            Tp_Transaction::create([
                ...
                'remark' => "Level 1 Ref Bonus as shopping wallet " . auth::user()->name,
            ]);
          ...
          $array=User::all();
          $parent=Auth::user()->id;
          $this->getAncestors($array, $deposit_amount, $parent, 1,auth::user()->id);
    }
...
echo'
 ...
 public function walletreport($type){
        ...
        return view('user.walletreport')
            ->with(array(

                't_history' => Tp_Transaction::select('users.name', 'tp__transactions.amount', 'tp__transactions.type', 'tp__transactions.plan', 'tp__transactions.created_at', 'users.email', 'tp__transactions.remark')
                    ->where('user', Auth::user()->id)
                    ->where('tp__transactions.type', '=', $type)
                    ->rightJoin('users', 'users.id', '=', 'tp__transactions.fromUser')
