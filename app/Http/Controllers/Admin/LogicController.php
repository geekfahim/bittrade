...
class LogicController extends Controller
{
    ...
    use AuthorizesRequests, DispatchesJobs, ValidatesRequests, CPTrait;
     ...
     public function addplan(Request $request){
        ...
        if($request['expiration'] == 'manual'){
            $plan->expiration= $request['invDurationValue'];
        }
        $plan->type= 'Main';
        $plan->save();
    ...
    public function topup(Request $request){
        ...
        $shopping_wallet_bal=$user->shopping_wallet_bal;
        ...
        if($request['t_type']=="Credit") {
          ...
          }elseif ($request['type']=="Shopping_Wallet") {

            $dp = new ShoppingWalletHistory();
            $dp->type = ShoppingWalletHistory::TYPE_ADDED;
            $dp->amount = $request['amount'];
            $dp->payment_mode = 'Admin';
            $dp->status = ShoppingWalletHistory::STATUS_COMPLETED;
            $dp->proof = '';
            $dp->user_id = $request['user_id'];
            $dp->save();

            User::where('id', $request['user_id'])->update([
              'shopping_wallet_bal'=> $shopping_wallet_bal + $request->amount,
            ]);
          }
         ...
         Tp_Transaction::create([
          ...
          if ($request['type']=="Bonus") {
            User::where('id', $request['user_id'])
              ...
              if($shopping_wallet_bal < $request->amount) {
                return redirect()->back()->with('message', 'Low shopping wallet balance !');
              }

              $dp = new ShoppingWalletHistory();
              $dp->type = ShoppingWalletHistory::TYPE_DEBIT;
              $dp->amount = $request['amount'];
              $dp->payment_mode = 'Admin';
              $dp->status = ShoppingWalletHistory::STATUS_COMPLETED;
              $dp->proof = '';
    ...
    public function edituser(Request $request){
      ...
      public function clearacct(Request $request, $id){
        ...
        //Delete deposit
        public function delshopping_wallet(Request $request, $id){
            ShoppingWalletHistory::where('id', $id)->delete();
            return redirect()->back()
            ->with('message', 'Shopping wallet history has been deleted!');
        }

           //process deposits
    public function pshopping_wallet($id){
      //confirm the users plan
      $deposit = ShoppingWalletHistory::where('id',$id)->first();
      $user = User::where('id',$deposit->user_id)->first();

      //add funds to user's account
      User::where('id',$user->id)->update([
        'shopping_wallet_bal' => $user->shopping_wallet_bal + $deposit->amount,
      ]);
      ...
      $objDemo->amount = $settings->currency.$deposit->amount;
      $objDemo->subject = "Deposit Successful";
          
      // Mail::to($user->email)->send(new DepositSuccess($objDemo));

      //update deposits
      ShoppingWalletHistory::where('id',$id)->update([
        'status' => ShoppingWalletHistory::STATUS_COMPLETED,
      ]);
     ...
     public function pdeposit($id){
       ...
       public function pwithdrawal($id){
        ...
        $settings=Settings::where('id', '=', '1')->first();
          ...
          // $objDemo->date = \Carbon\Carbon::Now();
          $objDemo->name = $user->name;
          $objDemo->amount = $settings->currency.$withdrawal->amount;
          $objDemo->subject = "Withdrawal Successful";
              
        Mail::to($user->email)->send(new WithdrawalSuccess($objDemo));
        return redirect()->back()
        ->with('message', 'Action Sucessful!');
        }
       ...
       public function trashplan($id)
       {
        ...
        $users=User::where('user_plan',$id)->get();
  ...
  public function saveuser(Request $request){
    ...
    $thisid = DB::table('users')->insertGetId( 
      [
        ...
        'ref_by'=>Auth::user()->id,
        'ref_id' => $ref_id,
        'ref_link' => $settings->site_address.'/ref/'.$ref_id,
...
function RandomStringGenerator($n) 
    ...
    public function deltest($id){
     ...
     //Verify user
  public function verifysystemuser(Request $request, $id){
     User::where('id', $id)
        ->update([
    ...
    return redirect()->route('manageusers')
    ->with('message', 'User has been Verified!');
  }  
