...
class SomeController extends Controller
{
  ...
  public function notification(){
    ...
    $user = Auth::user();
    return view('user.notification')->with('Notif', $user->notifications);
}
...
public function delnotif($id){
  ...
  function RandomStringGenerator($n) 
    ...
    //Return payment page
    public function add_money_shopping_wallet(Request $request){
     ...
     // return $secretkey;
      ...
      return redirect()->route('shopping_wallet_payment')
      ->with(array(
        'title' => 'Add Money',
        'settings' => Settings::where('id', '=', '1')->first(),
      ));
    ...
    public function transfer(Request $request){
      ...
      if($amount > Auth::user()->account_bal) {
      ...
      if($user->id == Auth::user()->id) {
        return redirect()->back()->with('message', 'You can\'t self transfer !');
      }

      $wdmethods = Wdmethod::where('type', 'transfer')->first();

      if(empty($wdmethods)) {
        return redirect()->back()->with('message', 'Transfer settings not found !');
      }

      if($wdmethods->minimum > $amount || $wdmethods->maximum < $amount) {
        return redirect()->back()->with('message', "Enter amount between $wdmethods->minimum and $wdmethods->maximum.");
      }

      $admin_amount = round(($wdmethods->charges_percentage/100)*$amount, 2);

      $transfer = new Transfer();
      $transfer->from_user_id = Auth::user()->id;
      $transfer->to_user_id = $user->id;
      $transfer->send_amount = $amount;
      $transfer->receive_amount = $amount - $admin_amount;
      $transfer->admin_amount = $admin_amount;
      $transfer->otp = rand(111111, 999999);
      $transfer->save();
      ...
      Mail::to(Auth::user()->email)->send(new TransferOTP($objDemo));

      return redirect()->route('transfer_otp', $transfer->id)
      ->with(array(
        'title' => 'Transfer OTP',
        'settings' => Settings::where('id', '=', '1')->first(),
      ));
    ...
    public function transferotp(Request $request){
      ...
      $transfer_update = Transfer::find($transfer->id);
      $transfer_update->status = Transfer::STATUS_COMPLETED;
      $transfer_update->save();

      $to_user = User::find($transfer->to_user_id);
      $to_user->account_bal += $transfer->receive_amount;
      $to_user->save();

      $from_user = User::find($transfer->from_user_id);
      $from_user->account_bal -= $transfer->send_amount;
      $from_user->save();

      $settings = Settings::where('id', '=', '1')->first();
      
      $objDemo = new \stdClass();
      $objDemo->name = $to_user->name;
      $objDemo->from_name = $from_user->name;
      $objDemo->amount = $settings->currency.$transfer->receive_amount;
      $objDemo->subject = "Transfer successful";
          
      Mail::to($to_user->email)->send(new TransferSuccess($objDemo));

      return redirect()->route('transfers')->with('message', 'Your transfer completed successfully.');
  }

   //payment route
   public function shopping_wallet_payment(Request $request){

    return view('user.shopping_wallet_payment')
    ->with(array(
  ...
  public function save_wallet_deposit(Request $request){
    ...
    $dp->status= ShoppingWalletHistory::STATUS_PENDING;
    $dp->proof = $proofname;
    ...
    return redirect()->route('shopping_wallet')->with('message', 'Action Successful! Please wait for system to validate this transaction.');
  }
  ...
  ->with('message', 'Action Sucessful! Please wait for system to validate this transaction.');
    ...
    //Save withdrawal requests
     public function withdrawal(Request $request){
        ...
        $objDemo->subject = "Withdrawal Successful";
            
        Mail::to($settings->contact_email)->send(new WithdrawalSuccess($objDemo));
